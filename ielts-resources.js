...
const resources = [
  {
    ...
    url:"https://www.cambridge.org/cambridgeenglish/catalog/cambridge-english-exams-ielts/official-cambridge-guide-ielts"
  },
  {
    ...
    url:"https://www.cambridge.org/in/cambridgeenglish/catalog/cambridge-english-exams-ielts/ielts-21-practice-tests/ielts-21-practice-tests-academic-students-book-answers-digital-pack?format=DO&isbn=9781009826723"
  },
  {
    ...
    url:"https://www.cambridge.org/in/cambridgeenglish/catalog/cambridge-english-exams-ielts/ielts-21-practice-tests/ielts-21-practice-tests-general-training-students-book-answers-digital-pack?format=DO&isbn=9781009826730"
  },
  {
...
function openResource(resource, trigger) {
  ...
  byId("resource-dialog-body").innerHTML = `<article class="resource-detail"><div class="resource-detail__visual">${visual(resource,true)}</div><div class="resource-detail__content"><span class="resource-detail__kicker">${escapeHtml(resource.source)} · ${tr("officialBadge")}</span><h2 id="resource-dialog-title">${escapeHtml(resource.title)}</h2><p class="resource-detail__description">${escapeHtml(local(resource.description))}</p><div class="resource-detail__facts">${facts}</div><h3>${tr("inside")}</h3><ul>${bullets}</ul><p class="resource-detail__best"><b>${tr("best")}:</b> ${escapeHtml(local(resource.best))}</p><div class="resource-detail__actions"><a href="${resource.url}" target="_blank" rel="noopener noreferrer"><span>${tr("official")}</span><span aria-hidden="true">↗</span></a><small>${tr("newTab")}</small></div></div></article>`;
